...
class GoogleTravelScraper:
    ...
    def channeling(self, ref):
        ...
        while not clicked and tries < MAX_RETRY:
            try:
                try:
                    menu_bt = wait.until(EC.element_to_be_clickable((By.XPATH,
                                                                     "(//div[@class=\'gm2-body-1 mapsConsumerUiSubviewSectionGm2Dropdown__label\'])[position()=1]")))
                except:
                    pass
    ...
    def scroll(self):
        ...
        while height[-1] - height[0] < 100000 and iter_index < 10000:
            ...
            if not \
            self.driver.find_elements_by_class_name("mapsConsumerUiSubviewSectionLoading__section-loading-spinner")[
                -1].is_displayed():
